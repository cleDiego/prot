        ...
        $('table > tbody  > tr').each(function(i, tr) {
            if ($(tr).find('td input[name="prot[]"]').length) {
                ...
                protList[i] = {
                    ...
                    'cliente': $(tr).find('td').eq(3).text().trim() ?  $(tr).find('td').eq(3).text().trim() : '',
                    'prioridade': $(tr).find('td').eq(4).text().trim() ? $(tr).find('td').eq(4).text().trim() : '',
                    'tipo': $(tr).find('td').eq(5).text().trim(),
                    'motivo': $(tr).find('td').eq(6).text().trim(),
                    'solicitacao': $(tr).find('td').eq(7).text().trim(),
                    'data_entrega': $(tr).find('td').eq(8).text().trim() ? $(tr).find('td').eq(8).text().trim() : '',
